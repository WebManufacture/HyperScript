...
# HyperScript
ГиперСкрипт - Новый, реактивный язык для Web, который не тащит за собой груз императивных языков прошлого века.
...

{
    ...
    //Также как и в JS в HS можно использовать доступ по номеру элемента
    a[0] + b[0];
    //Или доступ через точку . если в блоке есть именованные члены
    length = sqrt{vector.x * vector.x + vector.y * vector.y}
    //а вот как упростить эту запись используя контексты
    length = sqrt{vector{x * x + y * y}}
    //и даже количество скобочек можно уменьшить, используя цепочки контекстов
    length = sqrt + vector{x * x + y * y};
    //не очевиден + в данном месте, хотя все работает, т.к. vector{} - порождает блок
    length = sqrt << vector{x * x + y * y};
    с = a + b;
    //В этом случае, произойдет объединение блоков, и создается новый контекст
    //Теперь в контексте с будет лежать блок { 10, 20 } :) Волшебно не правда ли ?
    //А если написать вот так:
    = a + b
    //То числа 10, 20 как раз попадет в общий контекст
    //Оказывается в HyperScript над объектами можно совершать и другие операции!
    a - b //что правда неочевидный результат.
    a * b 
    //Также, существуют специальные виды объединений - >>, <<, @
}

```
В первой версии языка мы будем использовать специальные виды примесей
@iterator
